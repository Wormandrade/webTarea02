...
class Productos
{
    ...
    public function insertar($Codigo_Barras, $Nombre_Producto, $Graba_IVA, $estado)
    {
        try {
            $query = "INSERT INTO productos (Codigo_Barras, Nombre_Producto, Graba_IVA, estado) VALUES (?, ?, ?, ?)";
            $stmt = $this->con->prepare($query);
            $stmt->bind_param("ssi", $Codigo_Barras, $Nombre_Producto, $Graba_IVA, $estado);
            $stmt->execute();
    ...
    public function actualizar($idProductos, $Codigo_Barras, $Nombre_Producto, $Graba_IVA, $estado)
    {
        try {
